<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for Pure</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## About

The [Pure](https://github.com/sindresorhus/pure) prompt is available for many shells. This repo currently contains a theme for the [fish port of Pure](https://github.com/pure-fish/pure).

## Prerequisites

- [Pure](https://github.com/pure-fish/pure) prompt for fish

## Installation

Install with [Fisher](https://github.com/jorgebucaran/fisher):

```console
fisher install rose-pine/pure
```

Install manually:

Copy `conf.d/rose-pine-pure.fish` and the `conf.d/rose-pine-pure/` folder to `~/.config/fish/conf.d/`

## Usage

Add to your `~/.config/fish/config.fish`:

```fish
rose_pine_pure main  # or moon, dawn
```

To change variants, simply update the call in your config file and restart your shell, or run the function directly:

```fish
rose_pine_pure moon
```

### Auto-switching with fish's light/dark themes

Integrate with fish 4.3+ theme system to auto-switch between moon (dark) and dawn (light):

```fish
function __apply_theme --on-variable fish_terminal_color_theme
    if test "$fish_terminal_color_theme" = "dark"
        rose_pine_pure moon
    else
        rose_pine_pure dawn
    end
end
```

## Gallery

**Rosé Pine**

![Rosé Pine with Pure](assets/rose-pine.png)

**Rosé Pine Moon**

![Rosé Pine Moon with Pure](assets/rose-pine-moon.png)

**Rosé Pine Dawn**

![Rosé Pine Dawn with Pure](assets/rose-pine-dawn.png)

## Thanks to

- [simono](https://github.com/simono)
- [pure-fish](https://github.com/pure-fish) for the fish port of Pure
- [sindresorhus](https://github.com/sindresorhus) for Pure

## Contributing

Modify `source.fish` using Rosé Pine variables, then build variants:

<!-- #rose-pine-build -->

```sh
npx @rose-pine/build@0.9.1 -t ./source.fish -o ./conf.d/rose-pine-pure -f hex-ns
```

_Generated by [@rose-pine/build@0.9.1](https://github.com/rose-pine/build)_

<!-- /rose-pine-build -->
